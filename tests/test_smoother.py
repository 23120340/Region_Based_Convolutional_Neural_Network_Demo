...
class DebouncerTests(unittest.TestCase):
    ...
    def test_emits_after_three_votes(self) -> None:
        self.assertIsNone(self.debouncer.update(Prediction("open_case", 0.9)))
        self.assertIsNone(self.debouncer.update(Prediction("idle", 0.9)))
        self.assertIsNone(self.debouncer.update(Prediction("open_case", 0.8)))
        stable = self.debouncer.update(Prediction("open_case", 0.95))
        self.assertIsNotNone(stable)
        self.assertEqual(stable.action, "open_case")

    def test_sustained_action_only_emits_once(self) -> None:
        results = [
            self.debouncer.update(Prediction("open_case", 0.95))
            for _ in range(10)
        ]
    ...
    def test_low_confidence_is_not_counted(self) -> None:
        results = [
            self.debouncer.update(Prediction("open_case", 0.2))
            for _ in range(8)
        ]
    ...
    def test_idle_rearms_same_action(self) -> None:
        for _ in range(5):
            first = self.debouncer.update(Prediction("open_case", 0.9))
        self.assertIsNone(first)
        for _ in range(5):
            self.debouncer.update(Prediction("idle", 0.9))
        emitted = None
        for _ in range(5):
            emitted = self.debouncer.update(Prediction("open_case", 0.9)) or emitted
