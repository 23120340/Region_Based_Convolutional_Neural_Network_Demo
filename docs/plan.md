...
## 17. Repo cá nhân và baseline đơn giản khi chưa có bộ gá

Phần này dành cho giai đoạn chưa có PCB, IC hoặc dụng cụ lắp ráp chuyên dụng. Có thể dùng các vật dụng phổ biến như cốc, chai nước, thìa, bút, sách, hộp giấy, kéo và bàn tay để tạo bài toán nhận diện/tracking theo thời gian thực bằng webcam. Các repo dưới đây không thay thế dataset Assembly Tracker; chúng giúp dựng một prototype rẻ để kiểm tra capture, detection, tracking, action và event logic.

| Repo | Có thể làm với vật dụng đời thường | Nên học/tái sử dụng | Mức phù hợp |
|---|---|---|---|
| [Ultralytics](https://github.com/ultralytics/ultralytics) | Detect và track cốc, chai, bút, kéo hoặc các vật thể tự tạo nhãn bằng webcam | YOLO inference, tracking, export ONNX/TensorRT | Cao; dùng làm detector nền |
| [Roboflow Supervision](https://github.com/roboflow/supervision) | Vẽ vùng, đếm vật thể, theo dõi vật thể đi vào/ra vùng và tạo event | Annotator, zone, tracker và xử lý kết quả detector | Cao; phù hợp để dựng FSM baseline |
| [Norfair](https://github.com/tryolabs/norfair) | Theo dõi tâm hoặc bbox của vật thể từ detector đơn giản | Multi-object tracking nhẹ, không phụ thuộc YOLO cụ thể | Cao; thích hợp cho webcam và máy yếu |
| [CVZone](https://github.com/cvzone/cvzone) | Theo dõi bàn tay, ngón tay, pose và một số vật thể trong các bài toán tương tác với bàn | Các wrapper OpenCV/MediaPipe dễ thử nghiệm | Cao cho prototype; nên kiểm tra license trước khi đóng gói sản phẩm |
| [MediaPipe hand gesture recognition](https://github.com/kinivi/hand-gesture-recognition-mediapipe) | Nhận diện các cử chỉ như mở tay, nắm tay, chỉ tay hoặc ký hiệu tự định nghĩa | Hand landmarks và classifier theo chuỗi landmark | Trung bình-cao; dùng để thay action lắp ráp bằng action tay |
| [AI Virtual Mouse](https://github.com/Abhishek-jaiswal1/AI-Virtual-Mouse) | Dùng ngón trỏ và ngón cái để click/kéo, không cần vật mẫu đặc biệt | Ý tưởng nhận diện gesture, debounce và điều khiển theo thời gian thực | Trung bình; tham khảo event logic hơn là dùng trực tiếp |
| [DeepFace](https://github.com/serengil/deepface) | Nhận diện/xác thực người thực hiện trước khi bắt đầu một phiên demo | Face detection, recognition và logging theo subject | Thấp cho Assembly Tracker; chỉ dùng nếu cần phân biệt người |

### Ba đề tài prototype có thể làm ngay

1. **Desk Activity Tracker:** camera theo dõi cốc, chai nước và điện thoại; FSM gồm `EMPTY -> CUP_PLACED -> BOTTLE_ADDED -> DONE`. Thử lỗi đặt sai thứ tự, lấy vật ra, hoặc đưa tay qua vùng mà không đặt vật.
2. **Stationery Assembly Tracker:** dùng bút, thước, tẩy và hộp bút; kiểm tra việc đặt đúng vật vào đúng ô theo SOP. Đây là bài gần nhất với Assembly Tracker vì có object, zone, thứ tự và trạng thái sau hành động.
3. **Hand Gesture Workflow:** không cần vật thể, dùng các cử chỉ `open_hand -> point -> pinch -> thumbs_up` để mô phỏng một quy trình. Prototype này kiểm tra camera, temporal smoothing, debounce, FSM và cảnh báo sai thứ tự trước khi có detector vật thể.

### Lộ trình đề xuất

Nên bắt đầu bằng `Ultralytics + Supervision` nếu muốn nghiên cứu vật thể và vùng thao tác; chọn `MediaPipe + CVZone` nếu chưa có dữ liệu để train detector. Sau khi FSM và log chạy ổn với webcam, có thể thay vật dụng bằng linh kiện mô phỏng mà không phải viết lại toàn bộ pipeline. Các repo trên là baseline kỹ thuật, còn metric cuối cùng vẫn phải được đánh giá bằng dữ liệu quay theo người và theo full run như kế hoạch ở trên.
